...
pub fn build(b: *std.Build) void {
    ...
    // edgeboxc build generates zig-out/bundle_compiled.c for the target JS file
    wasm_static_exe.root_module.addCSourceFile(.{
        .file = .{ .cwd_relative = "zig-out/bundle_compiled.c" },
        .flags = quickjs_wasm_flags,
    });
    ...
    // All frozen functions stay in WASM/AOT (sandboxed) - no host exports
    wasm_static_exe.root_module.addCSourceFile(.{
        .file = .{ .cwd_relative = "zig-out/frozen_functions.c" },
        .flags = quickjs_wasm_flags,
    });
