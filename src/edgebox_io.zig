...
// ── Import Scanner + Module Resolver ──
// Scans .ts files for import paths, resolves relative imports.
// Runs in Zig dispatch thread before workers start.
// Results stored in a map: (importingFile + "\0" + moduleName) → resolvedPath
// Workers read via edgebox_get_resolved_module.

var resolved_modules: std.StringHashMapUnmanaged([]const u8) = .{};
var resolved_mutex: std.Thread.Mutex = .{};
var scan_count: u32 = 0;

/// Scan a source file for import paths. Returns them as a JSON array.
/// Uses byte scanning: finds `from '...'` and `from "..."` patterns.
fn scanImports(src: []const u8) !std.ArrayListUnmanaged([]const u8) {
    var imports: std.ArrayListUnmanaged([]const u8) = .{};
    var i: usize = 0;
    while (i + 6 < src.len) : (i += 1) {
        // Look for: from ' or from "
        if (src[i] == 'f' and src[i + 1] == 'r' and src[i + 2] == 'o' and src[i + 3] == 'm' and src[i + 4] == ' ') {
            var j = i + 5;
            // Skip whitespace
            ...
            if (j < src.len and (src[j] == '\'' or src[j] == '"')) {
                ...
                if (j < src.len and src[j] == quote and j > start) {
                    const path = src[start..j];
                    try imports.append(alloc, path);
                }
                i = j;
            }
        }
...
/// Try resolving a relative import to an absolute path.
/// Checks: path.ts, path/index.ts, path.d.ts, path/index.d.ts
fn resolveRelativeImport(dir: []const u8, module: []const u8) ?[]const u8 {
    const candidates = [_][]const u8{ ".ts", "/index.ts", ".d.ts", "/index.d.ts", ".tsx" };
    for (candidates) |ext| {
        const full = std.fmt.allocPrint(alloc, "{s}/{s}{s}", .{ dir, module, ext }) catch continue;
...
/// Scan all .ts files in a directory, extract imports, resolve relative paths.
/// Called from v8_pool.zig dispatch before workers start.
export fn edgebox_scan_and_resolve(dir_ptr: [*]const u8, dir_len: c_int) void {
    if (dir_len <= 0) return;
...
fn scanDir(dir: []const u8) !void {
    ...
    var it = d.iterate();
    while (it.next() catch null) |entry| {
        if (entry.kind == .file and std.mem.endsWith(u8, entry.name, ".ts") and !std.mem.endsWith(u8, entry.name, ".d.ts")) {
            const file_path = try std.fmt.allocPrint(alloc, "{s}/{s}", .{ dir, entry.name });
            // Read file from cache
            const content = cacheReadFile(file_path) catch continue;
            // Scan for imports
            var imports = scanImports(content) catch continue;
            defer imports.deinit(alloc);
            const file_dir = std.fs.path.dirname(file_path) orelse dir;
            for (imports.items) |imp| {
                // Only resolve relative imports
                if (imp.len > 0 and imp[0] == '.') {
                    if (resolveRelativeImport(file_dir, imp)) |resolved| {
                        // Store: key = moduleName + "\0" + importingFile → resolved path
                        const key = std.fmt.allocPrint(alloc, "{s}\x00{s}", .{ imp, file_path }) catch continue;
                        resolved_mutex.lock();
                        defer resolved_mutex.unlock();
                        resolved_modules.put(alloc, key, resolved) catch {};
                        scan_count += 1;
                    }
                }
            }
            ...
            const sub = try std.fmt.allocPrint(alloc, "{s}/{s}", .{ dir, entry.name });
...
/// Get a pre-resolved module path. Returns null if not pre-resolved.
export fn edgebox_get_resolved_module(module_ptr: [*]const u8, module_len: c_int, file_ptr: [*]const u8, file_len: c_int, out_len: *c_int) ?[*]const u8 {
    if (module_len <= 0 or file_len <= 0) { out_len.* = 0; return null; }
    const module = module_ptr[0..@intCast(module_len)];
    const file = file_ptr[0..@intCast(file_len)];
    const key = std.fmt.allocPrint(alloc, "{s}\x00{s}", .{ module, file }) catch { out_len.* = 0; return null; };
    defer alloc.free(key);
    resolved_mutex.lock();
    defer resolved_mutex.unlock();
    if (resolved_modules.get(key)) |resolved| {
        out_len.* = @intCast(resolved.len);
        return resolved.ptr;
    }
    out_len.* = 0;
    return null;
...
export fn edgebox_scan_stats() u32 {
    return scan_count;
}
