...
///   4. Run via wasmtime (JIT compilation)
///
...
// These provide no-op implementations for WASM since we don't have columnar storage
pub export fn native_registry_init() void {
    // No-op for WASM - native shapes not supported
}

pub export fn native_registry_count() c_int {
    return 0; // No native nodes in WASM
}

pub export fn native_node_register32(_: u32, _: i32, _: i32, _: i32, _: i32) ?*anyopaque {
    return null; // Registration not supported in WASM
}

pub export fn native_shapes_lookup(_: *anyopaque) ?*anyopaque {
    return null; // Shape lookup not supported in WASM
}
...
// Force export of frozen_module.frozen_init_c for C linkage
comptime {
...
pub fn main() !void {
    ...
    // The data section is reapplied during instantiation, which resets all global
...
fn initServeMode(allocator: std.mem.Allocator, args: []const [:0]u8) !void {
    ...
    const bytecode_ptr = bytecode.ptr;
    ...
    const frozen_count = frozen_module.frozen_init_c(@ptrCast(ctx));
    if (frozen_count < 0) {
...
///
/// NOTE: This function does NOT check g_serve_mode because the runtime may apply
...
fn executeBytecodeRaw(ctx: *qjs.JSContext) !void {
    ...
    const frozen_result2 = frozen_module.frozen_init_c(@ptrCast(ctx));
