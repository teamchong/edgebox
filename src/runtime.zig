...
pub fn main() !void {
    ...
    if (std.mem.eql(u8, cmd, "build")) {
        const app_dir = if (args.len > 2) args[2] else "examples/hello";
        try runBuild(allocator, app_dir);
    } else if (std.mem.eql(u8, cmd, "run")) {
        if (args.len < 3) {
            std.debug.print("Usage: edgebox run <script.js> [args...]\n", .{});
            std.process.exit(1);
        }
        ...
        // Direct script execution: edgebox script.js
...
fn printUsage() void {
    std.debug.print(
        ...
        \\
        \\Examples:
        \\  edgebox build                  Build default (examples/hello)
        \\  edgebox build examples/app     Build specific app
        \\  edgebox run script.js          Run script
        \\  edgebox script.js              Run script (shorthand)
        \\
    , .{});
}
