...
pub fn build(b: *std.Build) void {
    ...
    // edgeboxc build generates <source-dir>/zig-out/bundle_compiled.c for the target JS file
    const bundle_compiled_path = if (source_dir.len > 0)
        b.fmt("{s}/zig-out/bundle_compiled.c", .{source_dir})
    else
    ...
    const frozen_functions_path = if (source_dir.len > 0)
        b.fmt("{s}/zig-out/frozen_functions.c", .{source_dir})
    else
