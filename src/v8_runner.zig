...
pub fn main() !void {
    ...
    // Check for freeze-compiled output (from `edgebox compile`)
    // If available, use the SOA-transformed bundle instead of raw source.
    var compile_cache_dir_buf: [std.fs.max_path_bytes]u8 = undefined;
    const compile_cache_dir = std.fmt.bufPrint(&compile_cache_dir_buf,
        "zig-out/cache/{s}", .{script_path},
    ) catch null;

    g_use_compiled = false;
    var compiled_bundle: ?[]u8 = null;
    if (compile_cache_dir) |cache_dir| {
        var bundle_path_buf: [std.fs.max_path_bytes]u8 = undefined;
        const bundle_path = std.fmt.bufPrint(&bundle_path_buf,
            "{s}/bundle_hooked.js", .{cache_dir},
        ) catch null;
        if (bundle_path) |bp| {
            // Only load if a V8-compatible marker exists
            // (generated by future `edgebox compile --v8`)
            var marker_buf: [std.fs.max_path_bytes]u8 = undefined;
            const marker = std.fmt.bufPrint(&marker_buf, "{s}/.v8-compatible", .{cache_dir}) catch null;
            if (marker) |mp| {
                if (std.fs.cwd().statFile(mp) catch null) |_| {
                    compiled_bundle = std.fs.cwd().readFileAlloc(alloc, bp, 64 * 1024 * 1024) catch null;
                    if (compiled_bundle != null) {
                        g_use_compiled = true;
                        std.debug.print("[v8] Using freeze-compiled bundle\n", .{});
                    }
                }
            }
        }
    ...
    const script_code = if (compiled_bundle) |cb|
        cb
    else
        std.fs.cwd().readFileAlloc(alloc, script_path, 64 * 1024 * 1024) catch |err| {
...
fn runScript(alloc: std.mem.Allocator, script_code: []const u8, cache_bytes: ?[]const u8, abs_path: []const u8, display_name: []const u8, is_embedded: bool) !void {
    ...
    // Skip if using freeze-compiled bundle (already has SOA transforms).
