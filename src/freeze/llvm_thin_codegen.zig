//! Thin Codegen - LLVM IR generation for all functions (Tier B)
//!
//! Generates LLVM IR that calls llvm_rt_* runtime helpers for each QuickJS opcode.
//! This is the native binary path - only used when --with-binary is specified.
//! The worker-only path (default) uses standalone WASM codegen from llvm_codegen.zig.
...
const parent = @import("llvm_codegen.zig");
...
// Shared types and helpers from llvm_codegen.zig
const TargetInfo = parent.TargetInfo;
const CodegenError = parent.CodegenError;
const LLVMShardResult = parent.LLVMShardResult;
const ShardFunction = parent.ShardFunction;
const GeneratedFuncInfo = parent.GeneratedFuncInfo;
const native_target = parent.native_target;
const cvConstInt = parent.cvConstInt;
const declareNativeDispatchRegisterByIndex = parent.declareNativeDispatchRegisterByIndex;
const finalizeAndEmitShard = parent.finalizeAndEmitShard;
const generateShardInit = parent.generateShardInit;
const getOrDeclareIntrinsic = parent.getOrDeclareIntrinsic;
const getAtomStringStatic = parent.getAtomStringStatic;
// CV NaN-boxing constants
const CV_QNAN_INT = parent.CV_QNAN_INT;
const CV_TAG_MASK = parent.CV_TAG_MASK;
const CV_QNAN_PTR = parent.CV_QNAN_PTR;
const CV_QNAN_STR = parent.CV_QNAN_STR;
const CV_QNAN_SYMBOL = parent.CV_QNAN_SYMBOL;
const CV_QNAN_BIGINT = parent.CV_QNAN_BIGINT;
const CV_QNAN_FUNCBC = parent.CV_QNAN_FUNCBC;
const CV_UNDEFINED = parent.CV_UNDEFINED;
const CV_NULL = parent.CV_NULL;
const CV_TRUE = parent.CV_TRUE;
const CV_FALSE = parent.CV_FALSE;
const CV_PAYLOAD_MASK = parent.CV_PAYLOAD_MASK;
const CV_UNINITIALIZED = parent.CV_UNINITIALIZED;
...
// getAtomStringStatic imported from parent (llvm_codegen.zig)
...
fn emitVstackGetArrayEl(tctx: *ThinCodegenCtx, keep_array: bool) void {
    ...
    const jsv_struct_ty = native_target.jsvalueType();
