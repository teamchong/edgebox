...
pub fn build(b: *std.Build) void {
    ...
    native_static_exe.root_module.addIncludePath(b.path("vendor/libdeflate"));
    const native_static_is_x86 = target.result.cpu.arch == .x86_64 or target.result.cpu.arch == .x86;
    const native_static_libdeflate_flags: []const []const u8 = if (native_static_is_x86)
        &.{
            "-O3",
        ...
        &.{"-O3"};
    const native_static_cpu_features: []const u8 = if (native_static_is_x86) "x86/cpu_features.c" else "arm/cpu_features.c";
    native_static_exe.root_module.addCSourceFiles(.{
        ...
        .files = &.{
            ...
            "crc32.c",
            "utils.c",
            native_static_cpu_features,
        },
        .flags = native_static_libdeflate_flags,
    });
