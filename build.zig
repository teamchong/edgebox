...
pub fn build(b: *std.Build) void {
    ...
    // edgeboxc build generates zig-out/<source-dir>/bundle_compiled.c for the target JS file
    const bundle_compiled_path = if (source_dir.len > 0)
        b.fmt("zig-out/{s}/bundle_compiled.c", .{source_dir})
    else
    ...
    // All frozen functions stay in WASM/AOT (sandboxed) - no host exports
    const frozen_functions_path = if (source_dir.len > 0)
        b.fmt("zig-out/{s}/frozen_functions.c", .{source_dir})
    else
        "zig-out/frozen_functions.c";
    wasm_static_exe.root_module.addCSourceFile(.{
        .file = .{ .cwd_relative = frozen_functions_path },
        .flags = quickjs_wasm_flags,
    });
