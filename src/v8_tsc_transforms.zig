...
pub const transforms = [_]Transform{
    ...
    .{ .needle = "typeCount++;\n    result.id = typeCount;", .replacement = "typeCount++;\n    result.id = typeCount;\n    if(typeof __pc_typeFlags!=='undefined'&&typeCount<__pc_typeFlags.length)__pc_typeFlags[typeCount]=result.flags;\n    if(typeof __fidMap!=='undefined'){var __f=result.flags,__fid=__fidMap[__f];if(__fid===undefined){__fid=__fidNext++;__fidMap[__f]=__fid;}result.__fid=__fid;}" },
    ...
    .{ .needle = "function isTypeRelatedTo(source, target, relation) {\n    if (isFreshLiteralType(source)) {\n      source = source.regularType;\n    }\n    if (isFreshLiteralType(target)) {\n      target = target.regularType;\n    }\n    if (source === target) {\n      return true;\n    }", .replacement = "function isTypeRelatedTo(source, target, relation) {\n    if (isFreshLiteralType(source)) {\n      source = source.regularType;\n    }\n    if (isFreshLiteralType(target)) {\n      target = target.regularType;\n    }\n    if (source === target) {\n      return true;\n    }\n    var __tf,__sf;if(typeof __pc_typeFlags!=='undefined'){__tf=__pc_typeFlags[target.id];__sf=__pc_typeFlags[source.id];}else{__tf=target.flags;__sf=source.flags;}\n    if(__tf&1)return true;if(__sf&131072)return true;if(__sf&402653316&&__tf&4||__sf&296&&__tf&8||__sf&2112&&__tf&64||__sf&528&&__tf&16||__sf&12288&&__tf&4096)return true;" },
