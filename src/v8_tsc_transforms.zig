...
pub const transforms = [_]Transform{
    ...
    .{ .needle = "forEach(host.getSourceFiles(), (file) => checkSourceFileWithEagerDiagnostics(file));", .replacement = "{if(typeof __edgebox_precompute_relations==='function')__edgebox_precompute_relations(typeCount);try{var __fns=[isTypeRelatedTo,isSimpleTypeRelatedTo,checkTypeRelatedTo,getRelationKey,resolveStructuredTypeMembers,getPropertiesOfObjectType,getPropertiesOfType,forEachType,getObjectFlags,isFreshLiteralType];for(var __fi=0;__fi<__fns.length;__fi++){%PrepareFunctionForOptimization(__fns[__fi]);%OptimizeFunctionOnNextCall(__fns[__fi]);}}catch(e){}const __files=host.getSourceFiles();for(let __i=0;__i<__files.length;__i++){checkSourceFileWithEagerDiagnostics(__files[__i]);if(__i<30&&__i%5===4&&typeof __edgebox_precompute_relations==='function')__edgebox_precompute_relations(typeCount);}}" },
};
