  ...
  auto* isolate = creator.GetIsolate();
  {
    ...
    if (project && strlen(project) > 0) {
      ...
      snprintf(pre_parse, sizeof(pre_parse),
        ...
        "  __edgebox_write_stderr('[snapshot] pre-parsed ' + _prog.getSourceFiles().length + ' files\\n');"
        // Pre-compute diagnostics for ALL non-.d.ts files and cache in __ebDiagCache.
        // Workers restore with cached hashes+diags → skip checking unchanged files.
        "  var _checker2 = _prog.getTypeChecker();"
        "  var _srcF = _prog.getSourceFiles().filter(function(f){return !f.isDeclarationFile;});"
        "  var _dc = globalThis.__ebDiagCache;"
        "  if(_dc){var _cached=0;"
        "  for(var _i=0;_i<_srcF.length;_i++){"
        "    var _f=_srcF[_i],_fn=_f.fileName;"
        "    var _h=typeof __edgebox_hash==='function'?__edgebox_hash('sha256',_f.text):(function(){var _hh=0;for(var _ci=0;_ci<_f.text.length;_ci++)_hh=((_hh<<5)-_hh+_f.text.charCodeAt(_ci))|0;return ''+_hh;})();"
        "    var _d=_prog.getSemanticDiagnostics(_f);"
        "    var _msgs=[];"
        "    for(var _di=0;_di<_d.length;_di++){var _dd=_d[_di];"
        "      if(_dd.file){var _p=_dd.file.getLineAndCharacterOfPosition(_dd.start||0);"
        "        _msgs.push(_dd.file.fileName+'('+(_p.line+1)+','+(_p.character+1)+"
        "          '): error TS'+_dd.code+': '+ts.flattenDiagnosticMessageText(_dd.messageText,' '));}}"
        "    _dc.hashes[_fn]=''+_h;_dc.diags[_fn]=_msgs;_cached++;"
        "  }"
        "  __edgebox_write_stderr('[snapshot] pre-cached ' + _cached + ' file diagnostics\\n');}"
        "} catch(e) {"
