...
fn applyRecipeTransform(src: []const u8) ![]const u8 {
    ...
    // Property population disabled — 80ms overhead for eager population, structural
    // WASM check doesn't recover it. Need lazy population (only populate types that
    // are actually compared structurally).
