...
// Import frozen_init from frozen_functions.c (generated by edgebox-freeze)
// This hot-swaps slow interpreted JS functions with optimized native C functions
extern fn frozen_init(ctx: *qjs.JSContext) callconv(.c) c_int;
...
pub fn main() !void {
    ...
    logPrint("[MAIN] wizer_initialized={}\n", .{wizer_initialized});

    if (wizer_initialized) {
        logPrint("[MAIN] Using WIZER FAST PATH\n", .{});
    ...
    logPrint("[MAIN] Using SLOW PATH (no Wizer)\n", .{});
...
fn executeBytecode(context: *quickjs.Context) !void {
    const ctx = context.inner;
    logPrint("[executeBytecode] Starting\n", .{});
    ...
    logPrint("[executeBytecode] Bytecode loaded OK\n", .{});

    // Re-register native bindings AFTER loading bytecode (in case scope changes)
    logPrint("[executeBytecode] Re-registering native bindings\n", .{});
    ...
    logPrint("[executeBytecode] Initializing frozen functions\n", .{});
    _ = frozen_init(ctx);
